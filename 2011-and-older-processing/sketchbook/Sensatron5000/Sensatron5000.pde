...
import org.magiclamp.*;
import org.aardvark.processing.sensatron.*;
import org.aardvark.processing.delegate.*;
import org.aardvark.processing.*;
...
import javax.swing.*;
	...
	int STRAND_LENGTH = 24;
        ...
        // Number of lights along the top
        int SIDE_LENGTH = 14;
	...
	// on-screen light sim
        int TOP_LENGTH = STRAND_LENGTH - SIDE_LENGTH;
        final double TOP_LENGTH_RATIO = TOP_LENGTH / (double) STRAND_LENGTH;
	int SIZE = 500;
	int SPACING = 10;
...
//	  sideDisplay = eqDisplay;
	  ...
	  try {
          ...
          float[] ambienthsb = Color.RGBtoHSB((int)red(ambientColor), (int)green(ambientColor), (int)blue(ambientColor), null);
	  ambientHue = ambienthsb[0];
          ambientSat = ambienthsb[1];
          ambientBright = ambienthsb[2];
          System.out.println("Ambient HSB: " + ambientHue + ", " + ambientSat + ", " + ambientBright);
...
//	  }
	  ...
	  for (int strandNum = 0; strandNum < STRANDS; strandNum++) {
	    ...
	    for (int lightNum = 0; lightNum < STRAND_LENGTH; lightNum++) {
	      if (fullModeCheckbox.isChecked()) {
	        ...
	        if (lightNum < TOP_LENGTH) {
	          double light = lightNum / (TOP_LENGTH * 1.0);
	          if (invertTopCheckbox.isChecked()) light = 1 - light;
	          tween(strandNum, lightNum, topDisplay.getColor(strand, light));
	        } else {
                  double light = (lightNum - TOP_LENGTH) / (SIDE_LENGTH * 1.0);
	          if (invertFullCheckbox.isChecked()) light = 1 - light;
	          tween(strandNum, lightNum, sideDisplay.getColor(strand, light));
	        }
	      }
	...
	void tween(int strandNum, int lightNum, int targetColor) {
	  ...
	  float targetBlue = blue(targetColor);
          if (useAmbientColor) {
            float[] targethsb = Color.RGBtoHSB((int)targetRed, (int)targetGreen, (int)targetBlue, null);
            float targetHue = targethsb[0];
            float targetSat = targethsb[1];
            float targetBright = targethsb[2];
            if (targetBright < .05) {
              targetHue = ambientHue;
              targetSat = ambientSat;
              targetBright = ambientBright;
            } else if (ambientSat < .5f) {
              targetSat = targetSat + ((ambientSat - targetSat) * (1 - targetBright));
              targetBright = max(targetBright, ambientBright);
            } else {
              targetHue = targetHue + ((ambientHue - targetHue) * (1 - targetBright));
              targetSat = max(targetSat, ambientSat);
              targetBright = max(targetBright, ambientBright);
            }
            targetColor = Color.HSBtoRGB(targetHue, targetSat, targetBright);
      	    targetRed = red(targetColor);
      	    targetGreen = green(targetColor);
      	    targetBlue = blue(targetColor);
          }
...
//	    text(strand, x, y);
	...
	private void configureWidget1(LightDisplay newDisplay) {
		DisplayControlWidget newWidget = DisplayControlWidgetFactory.createWidget(newDisplay, this, WIDGET_1_X, WIDGET_1_Y);
		if (newWidget != null) {
	...
	private void configureWidget2(LightDisplay newDisplay) {
		DisplayControlWidget newWidget = DisplayControlWidgetFactory.createWidget(newDisplay, this, WIDGET_2_X, WIDGET_2_Y);
		if (newWidget != null) {
